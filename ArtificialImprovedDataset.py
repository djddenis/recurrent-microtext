...
class RepetitivePhrase:
    def __init__(self, reset_rate, spelling_change_ratio, phrase_change_ratio):
        ...
        self.message = ArtificialImprovedDatasetFactory.get_message()
    ...
    def mutate_phrase(starter):
        ...
        new_word = ArtificialImprovedDatasetFactory.get_word()
...
class ArtificialImprovedDatasetFactory:
    ...
    def get_word_count():
        ...
        for (length, prob) in enumerate(ArtificialImprovedDatasetFactory.WORD_COUNT_DISTRIBUTION, start=1):
            distributor -= prob
            if distributor <= 0:
                return length
    ...
    @staticmethod
    def get_word():
        length = ArtificialImprovedDatasetFactory.MEAN_WORD_LENGTH + random.randint(-3, 3)
        word = ''
    ...
    @staticmethod
    def get_message():
        word_count = ArtificialImprovedDatasetFactory.get_word_count()
        message = ''
        for count in xrange(word_count):
            message += ArtificialImprovedDatasetFactory.get_word() + ' '
        return message[:-1]
